...
public class DateUtil extends DateUtils {
     ...
     * @param stopDate 结束时间
     * @param week 取得星期几  比如1=星期一   2星期二   7 星期日    这个时间段的所有星期一日期
     * @return
     */
	public static List<java.util.Date> toWeekList(Date startDate,Date stopDate,int week){
		List<java.util.Date> list=new ArrayList<>();
		//取当前周的第一天
		DateTime start=	new DateTime(startDate).dayOfWeek().withMinimumValue();
		int  day=new Period(start,new DateTime(stopDate),PeriodType.days()).getDays();
		for (int i = 0; i < day; i++) {
			DateTime date=start.plusDays(i);
			if(date.getDayOfWeek()==week){
