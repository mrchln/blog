package com.flchy.blog.common.crawler;
...
public class TbPricesFieldRender implements CustomFieldRender {
	...
	public void render(HttpRequest request, HttpResponse response, BeanMap beanMap, SpiderBean bean, Field field) {
		...
		if (StringUtils.isNoneBlank(content)) {
			...
			String jsonResult = StringUtils.substringBetween(content, "jsonp525(", ");");
			Object json = JSON.parse(jsonResult);
			if (json != null) {
				Object mods = com.alibaba.fastjson.JSONPath.eval(json, "$.mods");
				Object itemlist = com.alibaba.fastjson.JSONPath.eval(mods, "$.itemlist");
				Object data = com.alibaba.fastjson.JSONPath.eval(itemlist, "$.data");
				Object auctions = com.alibaba.fastjson.JSONPath.eval(data, "$.auctions");
//				 System.out.println(auctions);
				List<ProductBrief> list = JSON.parseArray(auctions.toString(), ProductBrief.class);
