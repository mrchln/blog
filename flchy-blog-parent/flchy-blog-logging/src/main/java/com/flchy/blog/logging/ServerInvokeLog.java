...
@Component("serverInvokeLog")
public class ServerInvokeLog  implements AbstractLog ,ScheduledService{
	
	public static ServerInvokeLog getInstance() {
		return SpringContextHolder.getBean(ServerInvokeLog.class);
	}
	
	private static Logger logger = LoggerFactory.getLogger(ServerInvokeLog.class);
	...
	@Override
	public void start(Map<String, Object> invokeMap) {
		this.offer(invokeMap);
	}
...
//		logger.info("flushing {}...", this.getName());
		...
		for (Map<String, Object> message : messagesToFlush) {
			String shardingKey = String.valueOf( message.get("is_fail"));
			...
			try {
				...
				for(Map<String,Object> messageLog : shardingFlushEntries) {
					Object[] params = new Object[] {
							...
							messageLog.get("interface_id"),
							messageLog.get("invoke_cont"),
							messageLog.get("is_fail"),
							messageLog.get("err_msg"),
							messageLog.get("invoke_begin_time"),
							messageLog.get("invoke_end_time"),
...
//						
						...
						for (Map<String, Object> messageLog : shardingFlushEntries) {
							try {
								...
								pstmt.setInt(2, Integer.valueOf(messageLog.get("interface_id").toString()));
								pstmt.setString(3, String.valueOf(messageLog.get("invoke_cont")));
								pstmt.setInt(4, Integer.valueOf(messageLog.get("is_fail").toString()));
								pstmt.setString(5, String.valueOf(messageLog.get("err_msg")));
								pstmt.setObject(6,messageLog.get("invoke_begin_time"));
								pstmt.setObject(7,messageLog.get("invoke_end_time"));
								pstmt.setString(8, String.valueOf(messageLog.get("opr_user_id")));
								pstmt.setString(9, String.valueOf(messageLog.get("main_account")));
								pstmt.setString(10, String.valueOf(messageLog.get("server_ip")));
								pstmt.executeUpdate();
